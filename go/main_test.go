...
func TestTableWithMinor(t *testing.T) {
	var tests = []struct {
		str   string
		minor string
		want  string
	}{
		{"the quick fox in the bag", "bag", "The Quick Fox In The bag"},
		{"the quick fox in the bag", "the", "The Quick Fox In the Bag"},
		{"", "", ""},
		{"the quick fox in the bag", "", "The Quick Fox In The Bag"},
	}
	for _, e := range tests {
		got := titlecase.TitleCase(e.str, e.minor)
		if got != e.want {
			t.Errorf("TitleCase(%v, %v) = %v; want %v", e.str, e.minor, got, e.want)
		}
	}
}
